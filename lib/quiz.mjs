...
export function selectRandomQuestions(values, count, random = Math.random) {
  const selectionSize = Math.min(values.length, Math.max(0, Math.floor(count)));
  return shuffleQuestions(values, random).slice(0, selectionSize);
}
